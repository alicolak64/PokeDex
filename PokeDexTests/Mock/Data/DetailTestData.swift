...
struct DetailTestData {
    
    static let initalPokemonId = 1
    
    static let pokemonDetailDto = PokemonDetailDto(pokemonDetail: try! JSONDecoder().decode(PokemonDetail.self, from: DetailTestData.pokemonDetailJSON.data(using: .utf8)!))
    
    static let pokemonSpecies = try? JSONDecoder().decode(PokemonSpecies.self, from: DetailTestData.pokemonSpeciesJSON.data(using: .utf8)!)
    
    static let pokemonExtensionDto = PokemonDetailExtensionDto.init(pokemonDetailDto: pokemonDetailDto,description: "", color: getColorFromString(color: (pokemonDetailDto.types?.first) ?? "normal"))
        
    static let pokemonFlavorTexts = pokemonSpecies?.flavor_text_entries
