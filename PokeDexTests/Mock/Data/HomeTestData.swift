...
struct HomeTestData {
    ...
    static let allPokemons = try? JSONDecoder().decode(PokemonListResponse.self, from: HomeTestData.allPokemonJSON).results
    
    static let idSortedAllPokemonsDto = allPokemons!.map(PokemonDto.init)
    
    static let nameSortedAllPokemonsDto = idSortedAllPokemonsDto.sorted { $0.name < $1.name }
    
    static let searchNameText = "Bul"
    
    static let searchNumberText = "#64"
    
    static func searchPokemon(text: String, sortType: SortType) -> [PokemonDto] {
        
        var pokemons : [PokemonDto] = []
        
        if (text.first == "#") {
            
            pokemons = idSortedAllPokemonsDto.filter { pokemon in
